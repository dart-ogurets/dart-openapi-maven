...
main() {
  test('import conversion and compare works for issue-19', () {
    ...
    expect(w.types, [
      GeocodedWaypointTypesEnum.ADDRESS,
      GeocodedWaypointTypesEnum.NUMBER,
    ]);
    var x = w.copyWith();
    expect(w, x);
    expect(true, w == x);
  ...
  test(
    "List<AnyOf<MyApple,MyBanana>> - parsing json array with discriminator",
    () {
      final items = AnyOfMyAppleMyBanana.listFromJson(
        jsonDecode(_dummyDiscriminatorJson),
      );
      ...
      expect(
        items[1].discriminator,
        AnyOfDiscriminatorMyAppleMyBanana.MyBanana,
      );
